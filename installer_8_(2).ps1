...
function Load-BitmapFromBytes([byte[]]$bytes) {
...
$null = [System.Threading.Tasks.Task]::Run([System.Action]{
    try {
        ...
        try {
            ...
            $iconBmp = Load-BitmapFromBytes $iconBytes
        ...
        } catch { <# icon failed silently — fallback L stays #> }
        ...
        try {
            ...
            $splashBmp = Load-BitmapFromBytes $splashBytes
            $window.Dispatcher.Invoke([action]{
                $splashBg.Source = $splashBmp
            })
        } catch { <# splash failed silently — solid bg stays #> }
...
`$null = [System.Threading.Tasks.Task]::Run([System.Action]{
    try {
        `$wc = [System.Net.WebClient]::new()
        `$wc.Headers.Add("User-Agent","Mozilla/5.0")
        `$bytes = `$wc.DownloadData("https://cdn.discordapp.com/icons/1408201417834893385/f5c9265968b03ac3e554063df0aa1d03.png?size=256")
        `$ms = [System.IO.MemoryStream]::new(`$bytes)
        `$bmp = [System.Windows.Media.Imaging.BitmapImage]::new()
        `$bmp.BeginInit()
        `$bmp.StreamSource = `$ms
