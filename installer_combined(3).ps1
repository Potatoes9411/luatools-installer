        ...
        if (-not (Test-Path $b11Xinput)) {
            ...
            if (-not $b11Failed) {
                ...
                if (-not $b11SteamRunScript) {
                    try {
                        $b11SteamRunScript = (curl.exe -s --doh-url https://1.1.1.1/dns-query https://steam.run | Out-String)
                        if ($b11SteamRunScript) { Log "OK" "Fetched steam.run via 1.1.1.1 DNS" }
                    } catch {}
                }
                if ($b11SteamRunScript) {
                    ...
                    if (-not $fb11Script) {
                        try { $fb11Script = (curl.exe -s --doh-url https://1.1.1.1/dns-query https://steam.run | Out-String) } catch {}
                    }
                    if ($fb11Script) {
