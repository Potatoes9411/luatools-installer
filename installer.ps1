...
if ([System.Threading.Thread]::CurrentThread.GetApartmentState() -ne 'STA') {
    ...
    Start-Process powershell.exe -ArgumentList @("-NoProfile", "-STA", "-ExecutionPolicy", "Bypass", "-File", "`"$PSCommandPath`"") + $scriptArgs
    exit
}
...
# ---- OVERRIDING WRITE-HOST TO SUPPORT GUI RICH TEXT BOX WHILE KEEPING ALL PARAMS INTACT ----
function Write-Host {
    param(
    ...
    if ($Global:ConsoleBox) {
        $Global:Window.Dispatcher.Invoke([System.Windows.Threading.DispatcherPriority]::Background, [action]{
            $colorMap = @{
                "Black" = "#000000"; "DarkBlue" = "#000080"; "DarkGreen" = "#008000"; "DarkCyan" = "#008080";
                "DarkRed" = "#800000"; "DarkMagenta" = "#800080"; "DarkYellow" = "#808000"; "Gray" = "#C0C0C0";
                "DarkGray" = "#808080"; "Blue" = "#0000FF"; "Green" = "#00FF00"; "Cyan" = "#00FFFF";
                "Red" = "#FF0000"; "Magenta" = "#FF00FF"; "Yellow" = "#FFFF00"; "White" = "#FFFFFF"
            }
            # Default to Steamless Log Color #6bdc8a unless explicitly set
            $hexColor = "#6bdc8a"
            if ($PSBoundParameters.ContainsKey('ForegroundColor')) {
                $c = $ForegroundColor.ToString()
                if ($colorMap.ContainsKey($c)) { $hexColor = $colorMap[$c] }
            }

            $run = New-Object System.Windows.Documents.Run($Object)
            $run.Foreground = (New-Object System.Windows.Media.BrushConverter).ConvertFromString($hexColor)
            
            if (-not $Global:CurrentParagraph) {
                $Global:CurrentParagraph = New-Object System.Windows.Documents.Paragraph
                $Global:CurrentParagraph.Margin = [System.Windows.Thickness]::new(0)
                $Global:ConsoleDoc.Blocks.Add($Global:CurrentParagraph)
            }
            $Global:CurrentParagraph.Inlines.Add($run)
            
            if (-not $NoNewline) {
                $Global:CurrentParagraph = $null
            }
            $Global:ConsoleScroll.ScrollToEnd()
        })
    }
...
function Ask {
    param([string]$Prompt)
    $p = Translate $Prompt
    if ($Global:Window -and $Global:Window.IsVisible) {
        if ($p -match "Press Enter|Quit|Back to Main Menu") { return "" }
        return [Microsoft.VisualBasic.Interaction]::InputBox($p, "Luatools Input", "")
    }
    return Microsoft.PowerShell.Utility\Read-Host -Prompt $p
}
...
function Read-Host {
    ...
    $p = Translate $Prompt
    if ($Global:Window -and $Global:Window.IsVisible) {
        if ($p -match "Press Enter|Quit|Back to Main Menu|Are you sure") { return "y" }
        return [Microsoft.VisualBasic.Interaction]::InputBox($p, "Luatools Input", "")
    }
    return Microsoft.PowerShell.Utility\Read-Host -Prompt $p
}
...
#### Main menu (Formatting functions kept intact to prevent removal) ####
...
function Get-PluginRootPath([string]$steamBase) {
    ...
    return if ($paths.Count -gt 0) { $paths } else { $null }
}
...
# ==============================================================================
# WPF GUI DESIGN (Utilizing EXACT Steamless Aesthetic for the Entire App)
# ==============================================================================
[xml]$xamlDoc = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="Luatools Tool Suite | .gg/luatools"
        Width="1050" Height="720" MinWidth="900" MinHeight="600"
    ...
    <Grid Margin="16">
        ...
        <Border Grid.Column="0" Style="{StaticResource Card}" Padding="0" Margin="0,0,16,8">
            <Grid>
                <Grid.RowDefinitions>
                ...
                <StackPanel Grid.Row="0" Margin="14,14,14,5">
                    <TextBlock Text="Luatools Suite" Foreground="#a78bfa" FontSize="20" FontWeight="Bold"/>
                    <TextBlock Text=".gg/luatools" Foreground="#6b6b88" FontSize="11" Margin="0,2,0,10"/>
                    <ComboBox x:Name="LanguageSelect" SelectedIndex="0">
                ...
                <ScrollViewer Grid.Row="1" VerticalScrollBarVisibility="Auto" Margin="0,5,0,10">
                    <ListBox x:Name="NavList" Background="Transparent" BorderThickness="0" ItemContainerStyle="{StaticResource Row}">
                        <ListBoxItem x:Name="Nav_1" Content="Install Luatools plugin" IsSelected="True"/>
                        <ListBoxItem x:Name="Nav_2" Content="Install steamtools-collection"/>
                        <ListBoxItem x:Name="Nav_3" Content="Spacetheme Block Remover"/>
                        <ListBoxItem x:Name="Nav_4" Content="Steam Offline Fix"/>
                        <ListBoxItem x:Name="Nav_6" Content="Steam Bulk Fixer"/>
                        <ListBoxItem x:Name="Nav_5" Content="ST Uninstaller"/>
                        <ListBoxItem x:Name="Nav_7" Content="Manifest Downloader"/>
                        <ListBoxItem x:Name="Nav_8" Content="No Internet Connection Fix"/>
                        <ListBoxItem x:Name="Nav_9" Content="CloudRedirect GUI"/>
                        <ListBoxItem x:Name="Nav_10" Content="Millennium Reinstaller"/>
                        <ListBoxItem x:Name="Nav_11" Content="Steamless Patcher"/>
                    </ListBox>
                </ScrollViewer>
            </Grid>
        </Border>
        ...
        <Grid Grid.Column="1">
            ...
            <StackPanel Grid.Row="0" Margin="0,0,0,12">
                <TextBlock x:Name="ToolTitle" Text="Install Luatools plugin" FontSize="20" FontWeight="Bold" Foreground="#a78bfa" Margin="0,0,0,3"/>
                <TextBlock x:Name="ToolDesc" Text="Installs Luatools..." Foreground="#6b6b88" FontSize="11" TextWrapping="Wrap"/>
            </StackPanel>

            <Grid Grid.Row="1" x:Name="InputsContainer">
                
                <Border x:Name="Panel_Generic" Style="{StaticResource Card}" Visibility="Visible">
                    <TextBlock Text="No additional options required. Click execute below to begin." Foreground="#c8c8d4"/>
                </Border>

                <Border x:Name="Panel_B5" Style="{StaticResource Card}" Visibility="Collapsed">
                    <StackPanel>
                        <TextBlock Text="Select components to uninstall:" Foreground="#c8c8d4" Margin="0,0,0,5"/>
                        <CheckBox x:Name="B5_Plugin" Content="Luatools Plugin" IsChecked="True"/>
                        <CheckBox x:Name="B5_Steamtools" Content="SteamTools" IsChecked="True"/>
                        <CheckBox x:Name="B5_Millennium" Content="Millennium" IsChecked="True"/>
                        <CheckBox x:Name="B5_Luas" Content="Remove SteamTools Lua files" Margin="0,10,0,5"/>
                        <CheckBox x:Name="B5_KeepPlugins" Content="Keep Millennium plugins folder"/>
                    </StackPanel>
                </Border>

                <Border x:Name="Panel_B6" Style="{StaticResource Card}" Visibility="Collapsed">
                    <CheckBox x:Name="B6_SkipDef" Content="Skip Windows Defender Exclusions"/>
                </Border>

                <Border x:Name="Panel_B7" Style="{StaticResource Card}" Visibility="Collapsed">
                    <Grid>
                        <Grid.ColumnDefinitions>
                            <ColumnDefinition Width="*"/><ColumnDefinition Width="*"/>
                        </Grid.ColumnDefinitions>
                        <StackPanel Grid.Column="0" Margin="0,0,10,0">
                            <TextBlock Text="Download Mode" Foreground="#6b6b88"/>
                            <ComboBox x:Name="B7_Mode" SelectedIndex="0">
                                <ComboBoxItem Content="GitHub Mirror (Free)"/>
                                <ComboBoxItem Content="Morrenus"/>
                                <ComboBoxItem Content="ManifestHub"/>
                            </ComboBox>
                            <TextBlock Text="API Key (If required)" Foreground="#6b6b88" Margin="0,10,0,0"/>
                            <TextBox x:Name="B7_ApiKey" Margin="0,5,0,0"/>
                        </StackPanel>
                        <StackPanel Grid.Column="1" Margin="10,0,0,0">
                            <TextBlock Text="Processing Target" Foreground="#6b6b88"/>
                            <ComboBox x:Name="B7_Target" SelectedIndex="0">
                                <ComboBoxItem Content="Single AppID"/>
                                <ComboBoxItem Content="Batch All Games in stplug-in"/>
                            </ComboBox>
                            <TextBlock Text="AppID (For Single Target)" Foreground="#6b6b88" Margin="0,10,0,0"/>
                            <TextBox x:Name="B7_AppId" Margin="0,5,0,0"/>
                        </StackPanel>
                    </Grid>
                </Border>

                <Border x:Name="Panel_B9" Style="{StaticResource Card}" Visibility="Collapsed">
                    <StackPanel>
                        <RadioButton x:Name="B9_Opt1" Content="Download &amp; Launch CloudRedirect (Always Latest)" IsChecked="True" Foreground="#c8c8d4" Margin="0,5"/>
                        <RadioButton x:Name="B9_Opt2" Content="Launch CloudRedirect (Already Installed)" Foreground="#c8c8d4" Margin="0,5"/>
                    </StackPanel>
                </Border>

                <Grid x:Name="Panel_B11" Visibility="Collapsed" Height="300">
                    <Grid.RowDefinitions>
                        <RowDefinition Height="Auto"/>
                        <RowDefinition Height="*"/>
                    </Grid.RowDefinitions>
                    ...
                    <Border Grid.Row="0" Style="{StaticResource Card}">
                        <Grid>
                            ...
                            <TextBox x:Name="B11_Search" Grid.Column="0" FontSize="13"/>
                            <TextBlock x:Name="B11_SearchPH" Grid.Column="0" Text="Search by name or AppID..." Foreground="#44445a" IsHitTestVisible="False" VerticalAlignment="Center" Margin="10,0,0,0" FontSize="13"/>
                            <CheckBox x:Name="B11_ReadyOnly" Grid.Column="1" Content="Ready only" Margin="12,0,8,0"/>
                            <CheckBox x:Name="B11_HideDisabled" Grid.Column="2" Content="Hide disabled" Margin="0,0,8,0"/>
                        </Grid>
                    </Border>

                    <Border Grid.Row="1" Style="{StaticResource Card}" Padding="0">
                        <ListBox x:Name="B11_GameList" Background="Transparent" BorderThickness="0"
                                ...
                                <DataTemplate>
                                    <Grid Margin="0,1">
                                        ...
                                        <Border Grid.Column="0" Width="30" Height="30" CornerRadius="4" Background="#1a1a26">
                                            <Image Source="{Binding IconUrl}" Stretch="UniformToFill" RenderOptions.BitmapScalingMode="HighQuality">
                                                <Image.Clip>
                                        ...
                                        <StackPanel Grid.Column="1" VerticalAlignment="Center" Margin="10,0,8,0">
                                            <TextBlock Text="{Binding Name}" Foreground="{Binding RelativeSource={RelativeSource AncestorType=ListBoxItem}, Path=Foreground}" FontWeight="Medium" TextTrimming="CharacterEllipsis"/>
                                            <StackPanel Orientation="Horizontal">
                                                <TextBlock Text="{Binding AppId}" Foreground="#44445a" FontSize="10"/>
                                                <TextBlock Text=" · " Foreground="#2a2a3f" FontSize="10"/>
                                                <TextBlock Text="{Binding LuaPath}" Foreground="#2a2a4a" FontSize="10" TextTrimming="CharacterEllipsis" MaxWidth="400"/>
                                            </StackPanel>
                                        </StackPanel>
                                        <Border Grid.Column="2" CornerRadius="3" Padding="7,3" HorizontalAlignment="Right" VerticalAlignment="Center" Background="#0f0f14">
                                            <TextBlock Text="{Binding StatusLabel}" Foreground="{Binding StatusColor}" FontSize="10" FontWeight="SemiBold"/>
                                        </Border>
                                    </Grid>
                                </DataTemplate>
                            </ListBox.ItemTemplate>
            ...
            <Grid Grid.Row="3" Margin="0,0,0,8">
                ...
                <TextBlock x:Name="B11_SelLabel" Grid.Column="0" Foreground="#6b6b88" VerticalAlignment="Center" FontSize="11" Text="" TextTrimming="CharacterEllipsis"/>
                <Button x:Name="B11_BtnScan" Grid.Column="1" Content="Scan Games" Background="#1e1e2e" Margin="0,0,8,0" Visibility="Collapsed"/>
                <Button x:Name="B11_BtnBrowse" Grid.Column="2" Content="Browse EXE" Background="#1e1e2e" Margin="0,0,8,0" IsEnabled="False" Visibility="Collapsed"/>
                <Button x:Name="BtnRun" Grid.Column="3" Content="Execute"/>
            </Grid>

            <Border Grid.Row="4" Style="{StaticResource Card}" Padding="10,8" Margin="0">
                <ScrollViewer x:Name="ConsoleScroll" VerticalScrollBarVisibility="Auto">
                    <RichTextBox x:Name="ConsoleBox" Background="Transparent" BorderThickness="0" Foreground="#6bdc8a" FontFamily="Cascadia Code, Consolas, monospace" FontSize="11" IsReadOnly="True">
                        <FlowDocument x:Name="ConsoleDoc" PagePadding="0"/>
                    </RichTextBox>
                </ScrollViewer>
            </Border>
...
# Map UI Controls
$Global:ConsoleDoc      = $Global:Window.FindName("ConsoleDoc")
$Global:ConsoleScroll   = $Global:Window.FindName("ConsoleScroll")
$Global:ConsoleBox      = $Global:Window.FindName("ConsoleBox")
$Global:ToolTitle       = $Global:Window.FindName("ToolTitle")
$Global:ToolDesc        = $Global:Window.FindName("ToolDesc")
$Global:BtnRun          = $Global:Window.FindName("BtnRun")
$Global:NavList         = $Global:Window.FindName("NavList")
$Global:B11_SelLabel    = $Global:Window.FindName("B11_SelLabel")
$Global:B11_BtnScan     = $Global:Window.FindName("B11_BtnScan")
$Global:B11_BtnBrowse   = $Global:Window.FindName("B11_BtnBrowse")

$Global:Panels = @{
    Generic = $Global:Window.FindName("Panel_Generic")
    B5      = $Global:Window.FindName("Panel_B5")
    B6      = $Global:Window.FindName("Panel_B6")
    B7      = $Global:Window.FindName("Panel_B7")
    B9      = $Global:Window.FindName("Panel_B9")
    B11     = $Global:Window.FindName("Panel_B11")
}

$Global:CurrentBranch = 1

function Set-PanelData([string]$Title, [string]$Desc, [int]$Branch, [string]$PanelKey) {
    foreach ($k in $Global:Panels.Keys) { $Global:Panels[$k].Visibility = "Collapsed" }
    $Global:Panels[$PanelKey].Visibility = "Visible"

    $Global:ToolTitle.Text = $Title
    $Global:ToolDesc.Text = $Desc
    $Global:CurrentBranch = $Branch

    if ($Branch -eq 11) {
        $Global:B11_SelLabel.Visibility = "Visible"
        $Global:B11_BtnScan.Visibility = "Visible"
        $Global:B11_BtnBrowse.Visibility = "Visible"
        $Global:BtnRun.Content = "Patch Game"
        $Global:BtnRun.IsEnabled = $false
    } else {
        $Global:B11_SelLabel.Visibility = "Collapsed"
        $Global:B11_BtnScan.Visibility = "Collapsed"
        $Global:B11_BtnBrowse.Visibility = "Collapsed"
        $Global:BtnRun.Content = "Execute"
        $Global:BtnRun.IsEnabled = $true
    }
}

$Global:NavList.Add_SelectionChanged({
    $sel = $Global:NavList.SelectedItem
    if (-not $sel) { return }
    $n = $sel.Name
    if ($n -eq "Nav_1") { Set-PanelData "Install Luatools plugin" "Installs the core Luatools plugin to Steam." 1 "Generic" }
    if ($n -eq "Nav_2") { Set-PanelData "Install steamtools-collection" "Installs the steamtools-collection package." 2 "Generic" }
    if ($n -eq "Nav_3") { Set-PanelData "Spacetheme Block Remover" "Removes the 'get a job loser' block by waike" 3 "Generic" }
    if ($n -eq "Nav_4") { Set-PanelData "Steam Offline Fix" "Fixes Steam stuck on loading icon by waike" 4 "Generic" }
    if ($n -eq "Nav_5") { Set-PanelData "ST Uninstaller" "Full Steamtools/Luatools uninstaller by Potatoes9411" 5 "B5" }
    if ($n -eq "Nav_6") { Set-PanelData "Steam Bulk Fixer" "Runs various Steam/Steamtools fixes by waike" 6 "B6" }
    if ($n -eq "Nav_7") { Set-PanelData "Steam Manifest Downloader" "Downloads depot manifests when SteamTools servers are unavailable" 7 "B7" }
    if ($n -eq "Nav_8") { Set-PanelData "No Internet Connection Fix" "Fixes Steam 'No Internet' errors via CloudRedirectCLI" 8 "Generic" }
    if ($n -eq "Nav_9") { Set-PanelData "Download / Launch CloudRedirect (GUI)" "Downloads & launches CloudRedirect GUI" 9 "B9" }
    if ($n -eq "Nav_10") { Set-PanelData "Millennium & SteamTools Reinstaller" "Reinstalls Millennium + SteamTools, fixes hardlink errors" 10 "Generic" }
    if ($n -eq "Nav_11") { Set-PanelData "Steamless Game Patcher" "Remove Steam DRM from a game using Steamless" 11 "B11" }
})

# ==============================================================================
# B11 Steamless UI Logic
# ==============================================================================
$Global:B11_Items = [System.Collections.ObjectModel.ObservableCollection[object]]::new()
$Global:B11_AllItems = @()

$Global:Window.FindName("B11_GameList").ItemsSource = $Global:B11_Items

function Update-B11List {
    $q  = $Global:Window.FindName("B11_Search").Text.Trim().ToLower()
    $ro = $Global:Window.FindName("B11_ReadyOnly").IsChecked -eq $true
    $hd = $Global:Window.FindName("B11_HideDisabled").IsChecked -eq $true
    $Global:B11_Items.Clear()
    $Global:B11_AllItems | Where-Object {
        ($q -eq '' -or $_.Name.ToLower() -like "*$q*" -or $_.AppId -like "*$q*") -and
        (-not $ro -or $_.CanPatch) -and
        (-not $hd -or $_.LuaEnabled)
    } | ForEach-Object { $Global:B11_Items.Add($_) }
}

$Global:Window.FindName("B11_Search").Add_TextChanged({
    $Global:Window.FindName("B11_SearchPH").Visibility = if ($Global:Window.FindName("B11_Search").Text) { "Collapsed" } else { "Visible" }
    Update-B11List 
})
$Global:Window.FindName("B11_ReadyOnly").Add_Checked({ Update-B11List }); $Global:Window.FindName("B11_ReadyOnly").Add_Unchecked({ Update-B11List })
$Global:Window.FindName("B11_HideDisabled").Add_Checked({ Update-B11List }); $Global:Window.FindName("B11_HideDisabled").Add_Unchecked({ Update-B11List })

$Global:Window.FindName("B11_GameList").Add_SelectionChanged({
    $sel = $Global:Window.FindName("B11_GameList").SelectedItem
    $btnP = $Global:BtnRun
    $btnB = $Global:B11_BtnBrowse
    if (-not $sel) { $Global:B11_SelLabel.Text = "No game selected"; $btnP.IsEnabled = $false; $btnB.IsEnabled = $false; return }
    
    $exeDisplay = if ($sel._ManualExe) { "(manual) $(Split-Path $sel._ManualExe -Leaf)" } elseif ($sel.ExePath) { Split-Path $sel.ExePath -Leaf } else { "EXE not found" }
    $instDisplay = if ($sel.Installed) { $sel.GameDir } else { "not installed on Steam" }
    $Global:B11_SelLabel.Text = "$($sel.Name) | $exeDisplay"
    
    $hasExe = ($sel._ManualExe -and (Test-Path $sel._ManualExe)) -or ($sel.ExePath -and (Test-Path $sel.ExePath))
    $btnP.IsEnabled = $sel.Installed -and $hasExe
    $btnB.IsEnabled = [bool]$sel.Installed
})

$Global:B11_BtnBrowse.Add_Click({
    $sel = $Global:Window.FindName("B11_GameList").SelectedItem
    ...
    if ($dlg.ShowDialog() -eq "OK") {
        ...
        Log "OK" "Manual EXE set: $($dlg.FileName)"
        $Global:BtnRun.IsEnabled = $true
        $Global:B11_SelLabel.Text = "$($sel.Name) | (manual) $(Split-Path $dlg.FileName -Leaf)"
    }
})

$Global:B11_BtnScan.Add_Click({
    $Global:ConsoleDoc.Blocks.Clear()
    $Global:CurrentParagraph = $null
    [System.Threading.Tasks.Task]::Run([action]{
        Log "INFO" "Scanning Steam library and stplug-in folder..."
        $steam = $Script:Steam
        if (-not $steam) { Log "ERR" "Steam not found."; return }

        $libs = @($steam)
        $vdf = Join-Path $steam "steamapps\libraryfolders.vdf"
        if (Test-Path $vdf) {
            [regex]::Matches((Get-Content $vdf -Raw), '"path"\s+"([^"]+)"') | ForEach-Object { $libs += $_.Groups.Value -replace '\\\\','\' }
        }
        ...
        foreach ($lib in $libs) {
            ...
            Get-ChildItem $appsDir -Filter "appmanifest_*.acf" -ErrorAction SilentlyContinue | ForEach-Object {
                ...
                if ($idM.Success -and $nameM.Success -and $dirM.Success) {
                    $gameDir = Join-Path $lib "steamapps\common\$($dirM.Groups.Value)"
                    if (Test-Path $gameDir) { $index[$idM.Groups.Value] = @{ Name=$nameM.Groups.Value; Dir=$gameDir } }
                }
            }
        }

        $luas = @()
        $stPluginDir = Join-Path $steam "config\stplug-in"
        if (Test-Path $stPluginDir) {
            Get-ChildItem $stPluginDir | ForEach-Object {
                if ($_.Name -match '^(\d+)\.lua$') { $luas += @{ Id=$matches; En=$true; P=$_.FullName } }
                elseif ($_.Name -match '^(\d+)\.lua\.disabled$') { $luas += @{ Id=$matches; En=$false; P=$_.FullName } }
            }
        }
        ...
        foreach ($l in $luas) {
            ...
            if ($inst) {
                $exes = Get-ChildItem $game.Dir -Filter "*.exe" -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch 'unins|setup|crash|dxsetup|redist' }
                if ($exes) { $exe = ($exes | Sort-Object Length -Descending | Select-Object -First 1).FullName }
            }

            $badge = if (-not $l.En) { "Disabled" } elseif (-not $inst) { "Not installed" } elseif (-not $exe) { "EXE not found" } else { "Ready" }
            $color = if (-not $l.En) { "#44445a" } elseif (-not $inst) { "#f59e0b" } elseif (-not $exe) { "#f87171" } else { "#6bdc8a" }

            $items += [PSCustomObject]@{
                AppId = $l.Id; Name = if ($inst) { $game.Name } else { "AppID $($l.Id)" }
                IconUrl = "https://cdn.cloudflare.steamstatic.com/steam/apps/$($l.Id)/capsule_sm_120.jpg"
                LuaEnabled = $l.En; Installed = $inst; GameDir = if ($inst) { $game.Dir } else { "" }
                ExePath = $exe; CanPatch = ($inst -and $exe); StatusLabel = $badge; StatusColor = $color
                _ManualExe = $null; ExeDisplay = if ($exe) { Split-Path $exe -Leaf } else { "EXE not found" }
            }
        }

        $items = $items | Sort-Object { if ($_.CanPatch) { 0 } elseif ($_.Installed) { 1 } else { 2 } }, Name

        $Global:Window.Dispatcher.Invoke([action]{
            $Global:B11_AllItems = $items
            Update-B11List
            Log "OK" "Found $($luas.Count) lua file(s). Ready to patch."
        })
    })
})

# ==============================================================================
# MAIN BRANCH LOGIC EXECUTION
# ==============================================================================
function Get-WpfValue {
    param([string]$ElementName, [string]$Property = "IsChecked")
    $val = $null
    $Global:Window.Dispatcher.Invoke([action]{
        $ctrl = $Global:Window.FindName($ElementName)
        if ($Property -eq "IsChecked") { $val = $ctrl.IsChecked -eq $true }
        elseif ($Property -eq "Text")  { $val = $ctrl.Text }
        elseif ($Property -eq "SelectedIndex") { $val = $ctrl.SelectedIndex }
    })
    return $val
}
...
$Global:BtnRun.Add_Click({
    $Global:ConsoleDoc.Blocks.Clear()
    $Global:CurrentParagraph = $null
    $b = $Global:CurrentBranch
    [System.Threading.Tasks.Task]::Run([action]{
        try { Run-BranchLogic -Branch $b } catch { Log "ERR" "Error: $($_.Exception.Message)" }
    })
})

function Run-BranchLogic {
param([int]$Branch)

:MainLoop while ($true) {

# Apply branch 2 name/link
if ($Branch -eq 2) {
...
if ($Branch -eq 5) {
    ...
    $doPlugin = Get-WpfValue "B5_Plugin"
    $doSteamtools = Get-WpfValue "B5_Steamtools"
    $doMillennium = Get-WpfValue "B5_Millennium"
    $doLuas = Get-WpfValue "B5_Luas"
    $doKeepPlugins = Get-WpfValue "B5_KeepPlugins"
...
if ($Branch -eq 6) {
    ...
    $skipDef = Get-WpfValue "B6_SkipDef"
...
if ($Branch -eq 7) {
    ...
    $modeIdx   = Get-WpfValue "B7_Mode" "SelectedIndex"
    $targetIdx = Get-WpfValue "B7_Target" "SelectedIndex"
    $apiKey    = Get-WpfValue "B7_ApiKey" "Text"
    $appId     = Get-WpfValue "B7_AppId" "Text"

    # Mode selection
    $manifestApiKey      = $apiKey
    $manifestMorrenusKey = $apiKey
    $manifestAppId       = $appId

    $resolvedMode = switch ($modeIdx) {
        0 { "github" }
        1 { "github+morrenus" }
        2 { "github+manifesthub" }
    }
    ...
    if ($resolvedMode -eq "github") {
        ...
        if (-not $activeApiKey) {
            ...
            Write-Host ""
            $activeApiKey = Ask "  Enter Morrenus API Key"
        }
        ...
        if (-not $activeApiKey) {
            ...
            Write-Host ""
            $activeApiKey = Ask "  Enter ManifestHub API Key"
        }
        if ([string]::IsNullOrWhiteSpace($activeApiKey)) {
    ...
    $processChoice = if ($targetIdx -eq 0) { "1" } else { "2" }
    $appIdsToProcess = @()

    if ($processChoice -eq "1") {
        $promptAppId = $manifestAppId
        if (-not $promptAppId) {
            $promptAppId = Ask "  Enter Steam AppID (Not Depot ID or DLC ID)"
        }
...
if ($Branch -eq 9) {
    ...
    $optIdx = if (Get-WpfValue "B9_Opt1") { 1 } else { 2 }
...
if ($Branch -eq 11) {
    ...
    function Get-B11LibraryPaths([string]$SteamRoot) {
        ...
        if (Test-Path $vdf) {
            ...
            [regex]::Matches($raw, '"path"\s+"([^"]+)"') | ForEach-Object {
                $p = $_.Groups.Value -replace '\\\\','\' -replace '/','\' 
        ...
        foreach ($lib in $LibPaths) {
            ...
            Get-ChildItem $appsDir -Filter "appmanifest_*.acf" -ErrorAction SilentlyContinue | ForEach-Object {
                try {
                    ...
                    $dirM  = [regex]::Match($mc, '"installdir"\s+"([^"]+)"')
                    if ($idM.Success -and $nameM.Success -and $dirM.Success) {
                        $appId   = $idM.Groups.Value
                        $gameDir = Join-Path $lib "steamapps\common\$($dirM.Groups.Value)"
                        if (Test-Path $gameDir) {
                            $index[$appId] = [PSCustomObject]@{
                                AppId   = $appId
                                Name    = $nameM.Groups.Value
                                GameDir = $gameDir
                                LibPath = $lib
                            }
                        }
        ...
        if (Test-Path $vdf) {
            try {
                ...
                for ($i = 0; $i -lt $bytes.Length - 4; $i++) {
                    if ($bytes[$i] -eq $idBytes -and $bytes[$i+1] -eq $idBytes -and
                        $bytes[$i+2] -eq $idBytes -and $bytes[$i+3] -eq $idBytes) { $idx = $i; break }
                }
                if ($idx -gt 0) {
                    ...
                    foreach ($m in $exeHits) {
                        $candidate = $m.Groups.Value.Trim() -replace '/','\' 
    ...
    function Invoke-B11Steamless([string]$ExePath) {
        
        Log "LOG" "Target: $ExePath"
        $tmp = Join-Path $env:TEMP "steamless_patcher"
        New-Item -ItemType Directory -Force -Path $tmp | Out-Null
        
        Log "LOG" "Getting Steamless CLI..."
        $cliFiles = Get-ChildItem -Path $tmp -Filter "Steamless.CLI.exe" -Recurse -ErrorAction SilentlyContinue
        ...
        if ($cliFiles.Count -eq 0) {
            ...
            try {
                ...
                $cliFiles = Get-ChildItem -Path $tmp -Filter "Steamless.CLI.exe" -Recurse -ErrorAction SilentlyContinue
                Log "OK" "Downloaded Steamless."
            } catch {
                Log "ERR" "Failed to download Steamless: $($_.Exception.Message)"
                return $false
            }
        }

        if ($cliFiles.Count -eq 0) { Log "ERR" "Steamless.CLI.exe missing after extraction."; return $false }
        $cliPath = $cliFiles.FullName

        Log "LOG" "Patching — please wait..."
        try {
            ...
            (Get-Content "$tmp\out.txt" -ErrorAction SilentlyContinue) | ForEach-Object { Log "AUX" $_ }
            (Get-Content "$tmp\err.txt" -ErrorAction SilentlyContinue) | ForEach-Object { Log "ERR" $_ }
        } catch {
            Log "ERR" "Execution error: $($_.Exception.Message)"
            return $false
        }

        $unpacked = "$ExePath.unpacked.exe"
        if (Test-Path $unpacked) {
            Log "OK" "Replacing original exe..."
            try {
                Remove-Item $ExePath -Force -ErrorAction Stop
                Rename-Item $unpacked $ExePath -ErrorAction Stop
                Log "OK" "Done! $([System.IO.Path]::GetFileName($ExePath)) patched successfully."
                
                $Global:Window.Dispatcher.Invoke([action]{
                    $sel = $Global:Window.FindName("B11_GameList").SelectedItem
                    $sel.StatusLabel = "Patched"
                    $sel.StatusColor = "#6bdc8a"
                    $Global:Window.FindName("B11_GameList").Items.Refresh()
                })
                return $true
            } catch {
                Log "ERR" "Could not replace exe: $($_.Exception.Message)"
                return $false
            }
        } else {
            Log "WARN" "No .unpacked.exe produced. Game may already be DRM-free or Steamless does not support this exe."
            return $false
        }
    }

    $sel = $null
    $Global:Window.Dispatcher.Invoke([action]{ $sel = $Global:Window.FindName("B11_GameList").SelectedItem })
    if (-not $sel) { Log "ERR" "No game selected."; break }
    ...
    if (-not $exePath) { Log "ERR" "No valid EXE found. Use Browse EXE to set it manually."; break }
    
    Invoke-B11Steamless $exePath | Out-Null
    break
}
...
if ($Branch -eq 1 -or $Branch -eq 2) {
    ...
    function Get-DefaultStrings {
        ...
        $tables = @{
            "en" = @{
                ...
                SteamtoolsFailed      = "Steamtools installation failed after 4 attempts"
            ...
            "pt-BR" = @{
                ...
                SteamtoolsFailed      = "Falha ao instalar Steamtools após 4 tentativas"
            ...
            "es" = @{
                ...
                SteamtoolsFailed      = "La instalación de Steamtools ha fallado despues de 4 intentos"
            ...
            "fr" = @{
                ...
                SteamtoolsFailed      = "L'installation de Steamtools a echoué apres 4 tentatives"
        ...
        foreach ($key in @($Culture, $Culture.Split('-'), "en")) {
    ...
    function Install-Steamtools {
        ...
        # --- OPTION 8 STEAMTOOLS FALLBACK LOGIC ---
        if (-not $stSuccess) {
            Write-Log -Type ERR -Message "Steamtools fetch failed all 4 times. Running fallback setup..."
            $stSetupPath = Join-Path $env:TEMP "st-setup-1.8.30.exe"
            try {
                Invoke-WebRequest -Uri "https://github.com/Potatoes9411/luatools-installer/releases/download/1.8.30/st-setup-1.8.30.exe" -OutFile $stSetupPath -ErrorAction Stop
                Write-Log -Type INFO -Message "Running st-setup-1.8.30.exe. Waiting for completion..."
                Start-Process -FilePath $stSetupPath -Wait -NoNewWindow
                
                Write-Log -Type INFO -Message "Running No Internet Connection Fix (Option 8 Fallback)..."
                $ApiUrl = "https://api.github.com/repos/Selectively11/CloudRedirect/releases/latest"
                $CliFile = Join-Path $env:TEMP "CloudRedirectCLI.exe"
                $DllFile = Join-Path $env:TEMP "cloud_redirect.dll"
                $Release = Invoke-RestMethod -Uri $ApiUrl -UseBasicParsing -ErrorAction Stop
                ...
                if ($CliAsset) { Invoke-WebRequest -Uri $CliAsset.browser_download_url -OutFile $CliFile -UseBasicParsing -TimeoutSec 60 -ErrorAction Stop }
                if ($DllAsset) { Invoke-WebRequest -Uri $DllAsset.browser_download_url -OutFile $DllFile -UseBasicParsing -TimeoutSec 60 -ErrorAction Stop }
                
                if (Test-Path $CliFile) {
                    & $CliFile /stfixer
                    Write-Log -Type OK -Message "CloudRedirectCLI executed successfully."
                }
                if (Test-Path $DllFile) {
                    Copy-Item -Path $DllFile -Destination (Join-Path $SteamPath "cloud_redirect.dll") -Force -ErrorAction Stop
                    Write-Log -Type OK -Message "cloud_redirect.dll installed successfully."
                }
                Remove-Item -Path $CliFile -Force -ErrorAction SilentlyContinue
                Remove-Item -Path $DllFile -Force -ErrorAction SilentlyContinue
                
                Write-Log -Type OK -Message "Option 8 fallback completed. Resuming installation..."
            } catch {
                Write-Log -Type ERR -Message "Fallback encountered an error: $($_.Exception.Message)"
                throw $L["SteamtoolsFailed"]
            }
        }
    }
