...
$Translations = @{ 
    ...
    pt = @{ 
        ...
        "Expected: smm_ followed by 96 caracteres hexadecimais (100 caracteres no total)" = "Esperado: smm_ seguido de 96 caracteres hexadecimais (100 caracteres no total)"
...
# ---- OVERRIDING WRITE-HOST TO OUTPUT TO GUI RICH TEXT BOX IN ADDITION TO CONSOLE ----
function Write-Host {
    param(
...
# ==============================================================================
# WPF GUI DESIGN (Replaces the text-based menu prompting)
# ==============================================================================
[xml]$xamlDoc = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Background="#0f0f14" FontFamily="Segoe UI" FontSize="14">
    <Window.Resources>
    ...
    <Grid>
        ...
        <Border Grid.Column="0" Background="#16161f" Padding="15">
            <Grid>
                <Grid.RowDefinitions>
                ...
                <StackPanel Grid.Row="0" Margin="0,0,0,15">
                    <TextBlock Text="Luatools Tool Suite" Foreground="#a78bfa" FontSize="22" FontWeight="Bold"/>
                    <TextBlock Text=".gg/luatools" Foreground="#6b6b88" FontSize="13" Margin="0,2,0,10"/>
                </StackPanel>

                <ScrollViewer Grid.Row="1" VerticalScrollBarVisibility="Auto">
                    <StackPanel>
                        <TextBlock Text="INSTALL / UPDATE" Foreground="#44445a" FontSize="12" FontWeight="Bold" Margin="5,10,0,5"/>
                        <RadioButton x:Name="Nav_1" Content="Install Luatools plugin" Style="{StaticResource NavBtn}" IsChecked="True"/>
                        <RadioButton x:Name="Nav_2" Content="Install steamtools-collection" Style="{StaticResource NavBtn}"/>
                        
                        <TextBlock Text="FIXES" Foreground="#44445a" FontSize="12" FontWeight="Bold" Margin="5,15,0,5"/>
                        <RadioButton x:Name="Nav_3" Content="Spacetheme Block Remover" Style="{StaticResource NavBtn}"/>
                        <RadioButton x:Name="Nav_4" Content="Steam Offline Fix" Style="{StaticResource NavBtn}"/>
                        <RadioButton x:Name="Nav_6" Content="Steam Bulk Fixer" Style="{StaticResource NavBtn}"/>
                        
                        <TextBlock Text="OTHER" Foreground="#44445a" FontSize="12" FontWeight="Bold" Margin="5,15,0,5"/>
                        <RadioButton x:Name="Nav_5" Content="ST Uninstaller" Style="{StaticResource NavBtn}"/>
                        <RadioButton x:Name="Nav_7" Content="Manifest Downloader" Style="{StaticResource NavBtn}"/>
                        <RadioButton x:Name="Nav_8" Content="No Internet Fix" Style="{StaticResource NavBtn}"/>
                        <RadioButton x:Name="Nav_9" Content="CloudRedirect GUI" Style="{StaticResource NavBtn}"/>
                        <RadioButton x:Name="Nav_10" Content="Millennium Reinstaller" Style="{StaticResource NavBtn}"/>
                        
                        <TextBlock Text="PATCHER" Foreground="#44445a" FontSize="12" FontWeight="Bold" Margin="5,15,0,5"/>
                        <RadioButton x:Name="Nav_11" Content="Steamless Patcher" Style="{StaticResource NavBtn}"/>
                    </StackPanel>
                </ScrollViewer>
            </Grid>
        </Border>
        ...
        <Grid Grid.Column="1" Margin="25">
            ...
            <StackPanel Grid.Row="0" Margin="0,0,0,20">
                <TextBlock x:Name="ToolTitle" Text="Select an Option" FontSize="26" Foreground="White" FontWeight="Bold"/>
                <TextBlock x:Name="ToolDesc" Text="Click Execute to run the selected branch inside the console below." Foreground="#a0a0ab" TextWrapping="Wrap" Margin="0,8,0,0"/>
            </StackPanel>

            <StackPanel Grid.Row="1" Margin="0,0,0,20">
                <Button x:Name="BtnRun" Content="EXECUTE TOOL" Style="{StaticResource ActionBtn}" HorizontalAlignment="Left"/>
            </StackPanel>

            <Border Grid.Row="2" Background="#0a0a0f" BorderBrush="#1a1a26" BorderThickness="1" CornerRadius="6" Padding="10">
                <ScrollViewer x:Name="ConsoleScroll" VerticalScrollBarVisibility="Auto">
                    <RichTextBox x:Name="ConsoleBox" Background="Transparent" BorderThickness="0" IsReadOnly="True" FontFamily="Consolas">
...
function Set-PanelData([string]$Title, [string]$Desc, [int]$Branch) {
    $Global:ToolTitle.Text = $Title
    $Global:ToolDesc.Text = $Desc
    $Global:CurrentBranch = $Branch
}

$Global:Window.FindName("Nav_1").Add_Checked({ Set-PanelData "Install Luatools plugin" "Installs the core Luatools plugin to Steam." 1 })
$Global:Window.FindName("Nav_2").Add_Checked({ Set-PanelData "Install steamtools-collection" "Installs the steamtools-collection package." 2 })
$Global:Window.FindName("Nav_3").Add_Checked({ Set-PanelData "Spacetheme Block Remover" "Removes the 'get a job loser' block by waike" 3 })
$Global:Window.FindName("Nav_4").Add_Checked({ Set-PanelData "Steam Offline Fix" "Fixes Steam stuck on loading icon by waike" 4 })
$Global:Window.FindName("Nav_5").Add_Checked({ Set-PanelData "ST Uninstaller" "Full Steamtools/Luatools uninstaller by Potatoes9411" 5 })
$Global:Window.FindName("Nav_6").Add_Checked({ Set-PanelData "Steam Bulk Fixer" "Runs various Steam/Steamtools fixes by waike" 6 })
$Global:Window.FindName("Nav_7").Add_Checked({ Set-PanelData "Steam Manifest Downloader" "Downloads depot manifests when SteamTools servers are unavailable" 7 })
$Global:Window.FindName("Nav_8").Add_Checked({ Set-PanelData "No Internet Connection Fix" "Fixes Steam 'No Internet' errors via CloudRedirectCLI" 8 })
$Global:Window.FindName("Nav_9").Add_Checked({ Set-PanelData "Download / Launch CloudRedirect (GUI)" "Downloads & launches CloudRedirect GUI" 9 })
$Global:Window.FindName("Nav_10").Add_Checked({ Set-PanelData "Millennium & SteamTools Reinstaller" "Reinstalls Millennium + SteamTools, fixes hardlink errors" 10 })
$Global:Window.FindName("Nav_11").Add_Checked({ Set-PanelData "Steamless Game Patcher" "Remove Steam DRM from a game using Steamless" 11 })

# Execute Branch asynchronously inside UI context
$Global:BtnRun.Add_Click({
    $Global:ConsoleDoc.Blocks.Clear()
    $Global:CurrentParagraph = $null
    $b = $Global:CurrentBranch
    [System.Threading.Tasks.Task]::Run([action]{
        try { Run-BranchLogic -Branch $b } catch { Log "ERR" "Fatal Error: $($_.Exception.Message)" }
    })
})

# ==============================================================================
# MAIN BRANCH LOGIC ENCAPSULATION
# ==============================================================================
...
if ($Branch -eq 7) {
    ...
    if ($env:MANIFEST_MODE) {
        ...
        Write-Host ""
        do {
            $modeChoice = Ask "  Enter choice (1-3)"
    ...
    while ($true) {
        ...
        Write-Host ""
        do {
            $processChoice = Ask "  Enter choice (1-2)"
        ...
        Write-Host ""
        do {
            $nextChoice = Ask "  Enter choice (1-2)"
...
if ($Branch -eq 10) {
    ...
    while ($true) {
        Write-ReinstallMenu
        $b10Choice = Ask "Select an option"
...
if ($Branch -eq 11) {
    ...
    function Get-B11SteamlessCli {
        ...
        if ($cliFiles.Count -eq 0) { throw "Steamless.CLI.exe not found after extraction." }
        return $cliFiles.FullName
    }
...
function Invoke-Steamless([string]$ExePath, [System.Windows.Controls.TextBox]$LogBox) {
    ...
    if ($cliFiles.Count -eq 0) { SL "ERROR: Steamless.CLI.exe missing after extraction."; return $false }
...
if ($Branch -eq 1 -or $Branch -eq 2) {
    ...
    function Install-Steamtools {
        ...
        Write-Log -Type ERR -Message "Steamtools fetch failed all 5 times. Running fallback setup..."
