...
$runBtn.Add_Click({
    $runBtn.IsEnabled = $false
        ...
        function GS($m){ $stepLabel.Text = $m }
        try {
            if (-not $steamPath -or -not (Test-Path $steamPath)) { GL "[ERR]  Steam not found."; GS "Error — Steam not found."; $closeBtn.IsEnabled=$true; return }
            ...
            if ($roots.Count -eq 0) { GL "[ERR]  Spacetheme not found in any standard location."; GS "Error — Spacetheme not found."; $closeBtn.IsEnabled=$true; return }
...
$runBtn.Add_Click({
    $runBtn.IsEnabled=$false
        ...
        try {
            ...
            if(-not $sp -or -not (Test-Path $sp)){ GL "[ERR]  Steam path not found."; GS "Error — Steam not found."; $closeBtn.IsEnabled=$true; return }
...
$runBtn.Add_Click({
    ...
    $logBox.Text=""
        function GL($m){ $logBox.AppendText("$m`n"); $logBox.ScrollToEnd() }
        function GLW($m){ $logBox.AppendText("$m`n"); $logBox.ScrollToEnd() }
        function GLW($m){ $logBox.AppendText("$m`n"); $logBox.ScrollToEnd() }
        try {
            if (-not $steam) { GLW "[ERR]  Steam not found."; $runBtn.IsEnabled=$true; return }
...
function GL($m){ $logBox.AppendText("$m`n"); $logScroll.ScrollToBottom() }
function GS($m,$p){ $stepLabel.Text=$m; if($p -ge 0){$pb.Value=$p} }

$closeBtn.Add_Click({ $win.Close() })
$runBtn.Add_Click({
    $runBtn.IsEnabled=$false; $skipDef=$skipDefChk.IsChecked
        function GL($m){ $logBox.AppendText("$m`n"); $logScroll.ScrollToBottom() }
        function GS($m,$p=-1){ $stepLabel.Text=$m; if($p -ge 0){$pb.Value=$p} }
        try {
            GS "Finding Steam..." 5
            $sp=(Get-ItemProperty "HKCU:\Software\Valve\Steam" -EA SilentlyContinue).SteamPath
            if(-not $sp-or-not(Test-Path $sp)){ GL "[ERR]  Steam not found."; GS "Error — Steam not found." -1; $closeBtn.IsEnabled=$true; return }
            GL "[INFO] Steam path: $sp"
        ...
        try {
            ...
            if(-not $resp.can_make_requests){
                $morrKeyErr.Text="Daily limit reached ($($resp.daily_usage)/$($resp.daily_limit) requests). Try again tomorrow."
                    ...
                    $btnMorrKeyNext.IsEnabled=$true; $btnMorrKeyNext.Content="Validate Key and Continue  →"; return
            }
            ...
            $morrKeyErr.Text=$msg; $morrKeyErr.Visibility=[System.Windows.Visibility]::Visible; $btnMorrKeyNext.IsEnabled=$true; $btnMorrKeyNext.Content="Validate Key and Continue  →"
        }
    })
})
...
$runBtn.Add_Click({
    $runBtn.IsEnabled=$false
        ...
        try {
            ...
            if(-not $steam){ GL "[ERR]  Steam not found."; GS "Error — Steam not found." 0; $closeBtn.IsEnabled=$true; return }
            ...
            try {
                ...
                GS "Download failed — check log." 0; $closeBtn.IsEnabled=$true; return
            }
...
$downloadBtn.Add_Click({
    $downloadBtn.IsEnabled=$false; $launchBtn.IsEnabled=$false
        ...
        function GS($m){ $stepLabel.Text=$m }
        try {
            GS "Creating install directory..."
            try { New-Item -Path $cloudRedirectDir -ItemType Directory -Force -EA Stop | Out-Null; GL "[OK]   Directory ready: $cloudRedirectDir" }
            catch { GL "[ERR]  Could not create directory: $($_.Exception.Message)"; $downloadBtn.IsEnabled=$true; RefreshStatus; return }

            GS "Downloading CloudRedirect.exe from GitHub..."; GL "[INFO] Downloading latest CloudRedirect.exe..."
            try {
                Invoke-WebRequest -Uri $cloudRedirectUrl -OutFile $cloudRedirectExe -EA Stop
                GL "[OK]   Saved to: $cloudRedirectExe"
            } catch {
                GL "[ERR]  Download failed: $($_.Exception.Message)"; $downloadBtn.IsEnabled=$true; RefreshStatus; return
            }

            GS "Launching CloudRedirect..."; GL "[INFO] Starting CloudRedirect..."
            try { Start-Process -FilePath $cloudRedirectExe -EA Stop; GL "[OK]   CloudRedirect launched."; GS "CloudRedirect is running." }
            catch { GL "[ERR]  Failed to launch: $($_.Exception.Message)" }
        } catch { GL "[ERR]  $($_.Exception.Message)" }
        $downloadBtn.IsEnabled=$true; RefreshStatus
})
...
$runBtn.Add_Click({
    $runBtn.IsEnabled=$false
        ...
        function GS($m,$p=-1){ $stepLabel.Text=$m; if($p -ge 0){$pb.Value=$p} }
        try {
            if(-not $b10SP -or -not(Test-Path $b10SP)){ GL "[ERR]  Steam not found. Is Steam installed?"; GS "Error — Steam not found." 0; $closeBtn.IsEnabled=$true; return }
...
# ---- Steamless CLI patcher ----
function Invoke-Steamless([string]$ExePath, [System.Windows.Controls.TextBox]$LogBox) {
    function SL([string]$m) { $LogBox.AppendText("$m`n"); $LogBox.ScrollToEnd() }
    SL "Target: $ExePath"
    $tmp = Join-Path $env:TEMP "steamless_patcher"
    New-Item -ItemType Directory -Force -Path $tmp | Out-Null
    $cliFound = Get-ChildItem $tmp -Filter "Steamless.CLI.exe" -Recurse -EA SilentlyContinue | Select-Object -First 1
    if (-not $cliFound) {
        SL "Downloading Steamless v3.1.0.5..."
        try {
            Invoke-WebRequest "https://github.com/atom0s/Steamless/releases/download/v3.1.0.5/Steamless.v3.1.0.5.-.by.atom0s.zip" -OutFile "$tmp\steamless.zip" -EA Stop
            Expand-Archive "$tmp\steamless.zip" $tmp -Force
        } catch { SL "ERROR: $($_.Exception.Message)"; return $false }
        $cliFound = Get-ChildItem $tmp -Filter "Steamless.CLI.exe" -Recurse -EA SilentlyContinue | Select-Object -First 1
    }
    if (-not $cliFound) { SL "ERROR: CLI missing."; return $false }
    $cliPath = $cliFound.FullName
    SL "Patching..."
    try {
        Start-Process $cliPath -ArgumentList "`"$ExePath`"" -Wait -NoNewWindow -RedirectStandardOutput "$tmp\out.txt" -RedirectStandardError "$tmp\err.txt" -EA Stop
        (Get-Content "$tmp\out.txt" -EA SilentlyContinue) | ForEach-Object { SL $_ }
        (Get-Content "$tmp\err.txt" -EA SilentlyContinue) | ForEach-Object { SL "[err] $_" }
    } catch { SL "ERROR: $($_.Exception.Message)"; return $false }
    $unpacked = "$ExePath.unpacked.exe"
    if (Test-Path $unpacked) {
        SL "Replacing exe..."
        try { Remove-Item $ExePath -Force -EA Stop; Rename-Item $unpacked $ExePath -EA Stop; SL "Done!"; return $true }
        catch { SL "ERROR: $($_.Exception.Message)"; return $false }
    } else { SL "WARNING: No unpacked.exe produced. Game may already be DRM-free."; return $false }
}

# ---- Peron script patcher ----
function Invoke-PeronSteamless([string]$AppId, [System.Windows.Controls.TextBox]$LogBox) {
    function SL([string]$m) { $LogBox.AppendText("$m`n"); $LogBox.ScrollToEnd() }
    SL "Running Peron steamless.ps1 for AppID: $AppId"
    SL "Downloading script..."
    try {
        $sc = Invoke-RestMethod "https://raw.githubusercontent.com/Peron4TheWin/ps1collection/refs/heads/main/steamless.ps1" -EA Stop
        SL "Script downloaded. Running..."
        & ([scriptblock]::Create($sc)) -AppId $AppId
        SL "Done!"
        return $true
    } catch { SL "ERROR: $($_.Exception.Message)"; return $false }
}
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Width="900" Height="700" MinWidth="660" MinHeight="520"
        WindowStartupLocation="CenterScreen"
  ...
  <Window.Resources>
    ...
    <Style x:Key="PriBtn" TargetType="Button">
      <Setter Property="Background" Value="#c026d4"/>
      <Setter Property="Foreground" Value="White"/>
      <Setter Property="BorderThickness" Value="0"/>
      ...
      <Setter Property="Template">
        <Setter.Value>
          <ControlTemplate TargetType="Button">
            ...
            <ControlTemplate.Triggers>
              <Trigger Property="IsMouseOver" Value="True"><Setter Property="Background" Value="#e040fb"/></Trigger>
              <Trigger Property="IsEnabled" Value="False"><Setter Property="Background" Value="#1e1e2e"/><Setter Property="Foreground" Value="#44445a"/></Trigger>
            </ControlTemplate.Triggers>
          </ControlTemplate>
        </Setter.Value>
    ...
    <Style TargetType="TextBox">
      ...
      <Setter Property="BorderThickness" Value="1"/>
      <Setter Property="Padding" Value="8,6"/>
      <Setter Property="CaretBrush" Value="#e040fb"/>
    </Style>
    <Style TargetType="CheckBox">
      <Setter Property="Foreground" Value="#6b6b88"/>
      <Setter Property="VerticalAlignment" Value="Center"/>
  ...
  <Grid>
    ...
    <Border Grid.Row="0" Background="#0a0a12" Padding="16,14,16,10">
      <StackPanel>
        <StackPanel Orientation="Horizontal">
          <TextBlock Text="Steamless" FontSize="20" FontWeight="Bold" Foreground="#e040fb"/>
          <TextBlock Text=" Game Patcher" FontSize="20" FontWeight="Light" Foreground="#9575cd"/>
        </StackPanel>
        <TextBlock Foreground="#6b6b88" FontSize="11" TextWrapping="Wrap">
          Select a game and choose a patching method. Peron's method is recommended — it uses the AppID from the .lua filename directly. Steamless CLI requires the game to be installed.
        </TextBlock>
      </StackPanel>
    </Border>
    <Border Grid.Row="1" Background="#0d0d18" Padding="4,0">
      <StackPanel Orientation="Horizontal">
        <Button x:Name="TabPeron" Padding="18,10" FontSize="13" FontWeight="SemiBold" Cursor="Hand"
                Background="#12121e" Foreground="#e040fb" BorderThickness="0,0,0,2" BorderBrush="#e040fb">
          <Button.Template><ControlTemplate TargetType="Button">
            <Border Background="{TemplateBinding Background}" BorderBrush="{TemplateBinding BorderBrush}" BorderThickness="{TemplateBinding BorderThickness}" Padding="{TemplateBinding Padding}">
              <ContentPresenter HorizontalAlignment="Center" VerticalAlignment="Center"/>
            </Border>
          </ControlTemplate></Button.Template>
          ⚡  Peron's Method (Recommended)
        </Button>
        <Button x:Name="TabCli" Padding="18,10" FontSize="13" FontWeight="SemiBold" Cursor="Hand"
                Background="#0d0d18" Foreground="#6b6b88" BorderThickness="0,0,0,2" BorderBrush="Transparent">
          <Button.Template><ControlTemplate TargetType="Button">
            <Border Background="{TemplateBinding Background}" BorderBrush="{TemplateBinding BorderBrush}" BorderThickness="{TemplateBinding BorderThickness}" Padding="{TemplateBinding Padding}">
              <ContentPresenter HorizontalAlignment="Center" VerticalAlignment="Center"/>
            </Border>
          </ControlTemplate></Button.Template>
          🔧  Steamless CLI (Backup)
        </Button>
      </StackPanel>
    </Border>
    <Grid Grid.Row="2" Margin="12,10,12,12">
      <Grid.RowDefinitions>
        <RowDefinition Height="Auto"/>
        <RowDefinition Height="Auto"/>
        <RowDefinition Height="*"/>
      ...
      <Border Grid.Row="0" x:Name="BannerBorder" CornerRadius="6" Padding="12,8" Margin="0,0,0,8" Background="#22e040fb">
        <TextBlock x:Name="BannerText" Foreground="#e040fb" FontSize="11" TextWrapping="Wrap"
          Text="⚡  Peron's Method — downloads and runs steamless.ps1 using your game AppID from the .lua filename. Works for any game in your list. Just select and click Patch."/>
      </Border>
      <Border Grid.Row="1" Style="{StaticResource Card}">
        <Grid>
          ...
          <TextBlock x:Name="SearchPH" Grid.Column="0" Text="Search by name or AppID..." Foreground="#44445a" IsHitTestVisible="False" VerticalAlignment="Center" Margin="10,0,0,0" FontSize="13"/>
      ...
      <Border Grid.Row="2" Style="{StaticResource Card}" Padding="0" Margin="0,0,0,8">
        <ListBox x:Name="GameList" Background="Transparent" BorderThickness="0"
          ...
          <ListBox.ItemTemplate>
            <DataTemplate>
              <Grid Margin="0,1">
                <Grid.ColumnDefinitions><ColumnDefinition Width="36"/><ColumnDefinition Width="*"/><ColumnDefinition Width="110"/></Grid.ColumnDefinitions>
                <Border Grid.Column="0" Width="30" Height="30" CornerRadius="4" Background="#1a1a26">
                  <Image Source="{Binding IconUrl}" Stretch="UniformToFill" RenderOptions.BitmapScalingMode="HighQuality">
                    <Image.Clip><RectangleGeometry Rect="0,0,30,30" RadiusX="4" RadiusY="4"/></Image.Clip>
                  </Image>
                </Border>
                <StackPanel Grid.Column="1" VerticalAlignment="Center" Margin="10,0,8,0">
                  ...
                  <StackPanel Orientation="Horizontal">
                    ...
                    <TextBlock Text="{Binding LuaPath}" Foreground="#2a2a4a" FontSize="10" TextTrimming="CharacterEllipsis" MaxWidth="380"/>
                  </StackPanel>
                </StackPanel>
      ...
      <Grid Grid.Row="3" Margin="0,0,0,8">
        <Grid.ColumnDefinitions><ColumnDefinition Width="*"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="8"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="8"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions>
        <TextBlock x:Name="SelLabel" Grid.Column="0" Foreground="#6b6b88" VerticalAlignment="Center" FontSize="11" Text="Select a game above" TextTrimming="CharacterEllipsis"/>
        <Button x:Name="BrowseBtn"     Grid.Column="1" Style="{StaticResource SecBtn}"  Content="Browse EXE"       IsEnabled="False" Visibility="Collapsed"/>
        <Button x:Name="PatchPeronBtn" Grid.Column="3" Style="{StaticResource PriBtn}"  Content="⚡  Patch (Peron)" IsEnabled="False"/>
        <Button x:Name="PatchCliBtn"   Grid.Column="5" Style="{StaticResource SecBtn}"  Content="🔧  Patch (CLI)"   IsEnabled="False" Visibility="Collapsed"/>
      </Grid>
...
$patchPeronBtn = $window.FindName("PatchPeronBtn")
$patchCliBtn   = $window.FindName("PatchCliBtn")
$logBox        = $window.FindName("LogBox")
$tabPeron      = $window.FindName("TabPeron")
$tabCli        = $window.FindName("TabCli")
$bannerBorder  = $window.FindName("BannerBorder")
$bannerText    = $window.FindName("BannerText")

$script:activeTab = "peron"

function Set-Tab([string]$tab) {
    $script:activeTab = $tab
    $purple = [System.Windows.Media.SolidColorBrush]([System.Windows.Media.Color]::FromRgb(0xe0,0x40,0xfb))
    $dim    = [System.Windows.Media.SolidColorBrush]([System.Windows.Media.Color]::FromRgb(0x6b,0x6b,0x88))
    $active = [System.Windows.Media.SolidColorBrush]([System.Windows.Media.Color]::FromRgb(0x12,0x12,0x1e))
    $inact  = [System.Windows.Media.SolidColorBrush]([System.Windows.Media.Color]::FromRgb(0x0d,0x0d,0x18))
    $transp = [System.Windows.Media.Brushes]::Transparent
    if ($tab -eq "peron") {
        $tabPeron.Foreground = $purple; $tabPeron.Background = $active
        $tabPeron.SetValue([System.Windows.Controls.Control]::BorderBrushProperty, $purple)
        $tabCli.Foreground = $dim; $tabCli.Background = $inact
        $tabCli.SetValue([System.Windows.Controls.Control]::BorderBrushProperty, $transp)
        $bannerBorder.Background = [System.Windows.Media.SolidColorBrush]([System.Windows.Media.Color]::FromArgb(0x33,0xe0,0x40,0xfb))
        $bannerText.Foreground = $purple
        $bannerText.Text = "⚡  Peron's Method — downloads steamless.ps1 from Peron4TheWin/ps1collection and runs it with the AppID from your .lua filename. Works for any game in your list. No EXE needed."
        $patchPeronBtn.Visibility = [System.Windows.Visibility]::Visible
        $patchCliBtn.Visibility   = [System.Windows.Visibility]::Collapsed
        $browseBtn.Visibility     = [System.Windows.Visibility]::Collapsed
    } else {
        $violet = [System.Windows.Media.SolidColorBrush]([System.Windows.Media.Color]::FromRgb(0xa7,0x8b,0xfa))
        $tabCli.Foreground = $violet; $tabCli.Background = $active
        $tabCli.SetValue([System.Windows.Controls.Control]::BorderBrushProperty, $violet)
        $tabPeron.Foreground = $dim; $tabPeron.Background = $inact
        $tabPeron.SetValue([System.Windows.Controls.Control]::BorderBrushProperty, $transp)
        $bannerBorder.Background = [System.Windows.Media.SolidColorBrush]([System.Windows.Media.Color]::FromArgb(0x22,0xa7,0x8b,0xfa))
        $bannerText.Foreground = $violet
        $bannerText.Text = "🔧  Steamless CLI — downloads atom0s/Steamless and patches the game EXE. Requires the game to be installed on Steam. Use Browse EXE if auto-detection fails."
        $patchCliBtn.Visibility   = [System.Windows.Visibility]::Visible
        $patchPeronBtn.Visibility = [System.Windows.Visibility]::Collapsed
        $browseBtn.Visibility     = [System.Windows.Visibility]::Visible
    }
    Update-Buttons
}

$tabPeron.Add_Click({ Set-Tab "peron" })
$tabCli.Add_Click({   Set-Tab "cli" })
...
$logBox.Text = "Found $($b11AllItems.Count) lua file(s).  Ready: $ready  |  Not installed: $noInst  |  Disabled: $dis`nSteam root: $SteamRoot`n`nSelect a game above, then click Patch.`n"
...
function Update-List {
    ...
    $b11AllItems | Where-Object {
        ...
        (-not $ro -or $_.CanPatch) -and (-not $hd -or $_.LuaEnabled)
...
function Update-Buttons {
    ...
    if (-not $sel) { $patchPeronBtn.IsEnabled = $false; $patchCliBtn.IsEnabled = $false; $browseBtn.IsEnabled = $false; return }
    if ($script:activeTab -eq "peron") {
        $patchPeronBtn.IsEnabled = [bool]$sel.AppId
    } else {
        $hasExe = ($sel._ManualExe -and (Test-Path $sel._ManualExe)) -or ($sel.ExePath -and (Test-Path $sel.ExePath))
        $patchCliBtn.IsEnabled = $sel.Installed -and $hasExe
...
$gameList.Add_SelectionChanged({
    $sel = $gameList.SelectedItem
    if (-not $sel) { $selLabel.Text = "Select a game above"; Update-Buttons; return }
    $exeDisplay  = if ($sel._ManualExe) { "(manual) $([IO.Path]::GetFileName($sel._ManualExe))" }
                   elseif ($sel.ExePath) { $sel.ExePath } else { "EXE not found" }
    $instDisplay = if ($sel.Installed) { "installed" } else { "not installed" }
    $selLabel.Text = "$($sel.Name)  (AppID: $($sel.AppId))  —  $instDisplay  —  $exeDisplay"
    Update-Buttons
})
...
$browseBtn.Add_Click({
    ...
    if ($dlg.ShowDialog() -eq "OK") {
        ...
        $patchCliBtn.IsEnabled = $true
        $selLabel.Text = "$($sel.Name)  (AppID: $($sel.AppId))  —  (manual) $([IO.Path]::GetFileName($dlg.FileName))"
    }
})

$patchPeronBtn.Add_Click({
    $sel = $gameList.SelectedItem
    if (-not $sel) { return }
    $appId = $sel.AppId
    $patchPeronBtn.IsEnabled = $false
    $logBox.Text = ""
    ...
    function SL([string]$m) { $LogBox.AppendText("$m`n"); $LogBox.ScrollToEnd() }
        ...
        $ok = Invoke-PeronSteamless $appId $logBox
        $disp.Invoke([action]{
            $sel.StatusLabel = if ($ok) { "Patched" } else { "Failed" }
            $sel.StatusColor = if ($ok) { "#6bdc8a" } else { "#f87171" }
            $patchPeronBtn.IsEnabled = $true
        })
})

$patchCliBtn.Add_Click({
    $sel = $gameList.SelectedItem
    if (-not $sel) { return }
    ...
    if (-not $exePath) { $logBox.Text = "ERROR: No EXE found. Use Browse EXE.`n"; return }
    $patchCliBtn.IsEnabled = $false; $browseBtn.IsEnabled = $false
    $logBox.Text = ""
    $disp = $window.Dispatcher
        function Invoke-Steamless([string]$ExePath, [System.Windows.Controls.TextBox]$LogBox) {
    function SL([string]$m) { $LogBox.AppendText("$m`n"); $LogBox.ScrollToEnd() }
        ...
        $disp.Invoke([action]{
            $sel.StatusLabel = if ($ok) { "Patched" } else { "Failed" }
            $sel.StatusColor = if ($ok) { "#6bdc8a" } else { "#f87171" }
            $patchCliBtn.IsEnabled = $true; $browseBtn.IsEnabled = $true
        })
})
