...
if ($Branch -eq 1 -or $Branch -eq 2) {
    ...
    function Install-Steamtools {
        ...
        $raw = $null
        try { $raw = Invoke-RestMethod "https://raw.githubusercontent.com/Potatoes9411/steamtools-ps/refs/heads/main/st.ps1" -TimeoutSec 30 -ErrorAction Stop } catch {}
        if (!$raw) {
            try { $raw = (curl.exe -s --doh-url https://1.1.1.1/dns-query https://raw.githubusercontent.com/Potatoes9411/steamtools-ps/refs/heads/main/st.ps1 | Out-String) } catch {}
        }
        if (!$raw) {
            try { $raw = Invoke-RestMethod "https://luatools.vercel.app/st.ps1" -TimeoutSec 30 -ErrorAction Stop } catch {}
        }
        if (!$raw) {
            try { $raw = (curl.exe -s --doh-url https://1.1.1.1/dns-query https://luatools.vercel.app/st.ps1 | Out-String) } catch {}
        }
        if (!$raw) {
