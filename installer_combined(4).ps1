...
function Write-MainMenu {
    ...
    Write-MenuEntry "11" "SteamTools DLL Refresher" "" "Renames xinput1_4.dll to dwmapi.dll, restarts Steam, then restores the original name and re-runs SteamTools"
...
#### Branch 11: SteamTools DLL Refresher ####
if ($Branch -eq 11) {
    $Host.UI.RawUI.WindowTitle = "DLL Refresher | .gg/luatools"

    # ---- Branch 11: Inner functions ----

    function Get-B11SteamPath {
        $candidates = @()
        try { $r = Get-ItemProperty "HKLM:\SOFTWARE\WOW6432Node\Valve\Steam" -ErrorAction SilentlyContinue; if ($r.InstallPath) { $candidates += $r.InstallPath } } catch {}
        try { $r = Get-ItemProperty "HKCU:\Software\Valve\Steam" -ErrorAction SilentlyContinue; if ($r.SteamPath) { $candidates += $r.SteamPath } } catch {}
        $candidates += "C:\Program Files (x86)\Steam"
        $candidates += "C:\Program Files\Steam"
        return ($candidates | Select-Object -Unique | Where-Object { Test-Path (Join-Path $_ "steam.exe") } | Select-Object -First 1)
    }

    function Wait-B11SteamReady {
        # Wait until steam.exe is running AND its main window is responsive (not just briefly spawned)
        Log "INFO" "Waiting for Steam to fully load..."
        $timeout  = 120   # seconds max
        $elapsed  = 0
        $interval = 3

        # First: wait for steam process to appear
        ...
        # Then: wait for steamwebhelper (signals Steam UI is up) or a further 30s grace
        $elapsed2 = 0
        while ($elapsed2 -lt 60) {
            $webHelper = Get-Process -Name "steamwebhelper" -ErrorAction SilentlyContinue
            if ($webHelper) {
        ...
        Log "OK" "Steam appears fully loaded."
    }

    function Stop-B11Steam {
        param([string]$SteamPath)
        Log "INFO" "Closing Steam..."
        $steamExe = Join-Path $SteamPath "steam.exe"
        # Graceful close first
        if (Test-Path $steamExe) {
            Start-Process $steamExe -ArgumentList "-shutdown" -ErrorAction SilentlyContinue
            Start-Sleep -Seconds 4
        }
        # Force kill anything remaining
        $killNames = @("steam","steamwebhelper","steamservice","steamerrorreporter","GameOverlayUI","steamtours","millennium","millennium.luavm64")
    ...
    # ---- Main Branch 11 logic ----

    Clear-Host
    Sep
    Write-Host "  SteamTools DLL Refresher  |  .gg/luatools" -ForegroundColor Cyan
    Sep
    Blank
    Write-Host "  This will:" -ForegroundColor White
    Write-Host "  1. Rename xinput1_4.dll  ->  dwmapi.dll" -ForegroundColor DarkGray
    Write-Host "  2. Start Steam and wait for it to fully load" -ForegroundColor DarkGray
    Write-Host "  3. Close Steam again" -ForegroundColor DarkGray
    Write-Host "  4. Rename dwmapi.dll  ->  xinput1_4.dll" -ForegroundColor DarkGray
    Write-Host "  5. Run: irm steam.run | iex" -ForegroundColor DarkGray
    Blank
    Sep
    Blank

    $b11SteamPath = Get-B11SteamPath
    if (-not $b11SteamPath) {
        Log "ERR" "Steam installation not found. Please make sure Steam is installed."
        Blank
        Read-Host "Press Enter to go back to the menu"
    } else {

        $b11Xinput  = Join-Path $b11SteamPath "xinput1_4.dll"
        $b11Dwmapi  = Join-Path $b11SteamPath "dwmapi.dll"
        $b11SteamExe = Join-Path $b11SteamPath "steam.exe"

        # ---- Step 1: Verify xinput1_4.dll exists ----
        if (-not (Test-Path $b11Xinput)) {
            # xinput1_4.dll not found — check if it was left as dwmapi.dll from a previous interrupted run
            if (Test-Path $b11Dwmapi) {
                Log "WARN" "xinput1_4.dll not found, but dwmapi.dll exists — looks like a previous run was interrupted."
                Log "INFO" "Renaming dwmapi.dll -> xinput1_4.dll to restore state..."
                try {
                    Rename-Item -Path $b11Dwmapi -NewName "xinput1_4.dll" -ErrorAction Stop
                    Log "OK" "Restored xinput1_4.dll. Re-run option 11 to continue."
                } catch {
                    Log "ERR" "Rename failed: $($_.Exception.Message)"
                }
            } else {
                Log "ERR" "xinput1_4.dll not found in Steam folder: $b11SteamPath"
                Log "WARN" "SteamTools may not be installed, or the DLL has a different name."
            }
            Blank
            Read-Host "Press Enter to go back to the menu"
        } else {

            # Make sure Steam is fully closed before we rename
            Stop-B11Steam $b11SteamPath

            # ---- Step 2: Rename xinput1_4.dll -> dwmapi.dll ----
            Log "INFO" "Renaming xinput1_4.dll -> dwmapi.dll..."
            try {
                # Remove any existing dwmapi.dll first so Rename-Item doesn't conflict
                if (Test-Path $b11Dwmapi) {
                    Remove-Item $b11Dwmapi -Force -ErrorAction Stop
                    Log "WARN" "Existing dwmapi.dll removed."
                }
                Rename-Item -Path $b11Xinput -NewName "dwmapi.dll" -ErrorAction Stop
                Log "OK" "Renamed xinput1_4.dll -> dwmapi.dll"
            } catch {
                Log "ERR" "Rename failed: $($_.Exception.Message)"
                Blank
                Read-Host "Press Enter to go back to the menu"
            ...
            if (-not $b11Failed) {
                # ---- Step 3: Start Steam and wait for it to fully load ----
                Log "INFO" "Starting Steam..."
                Start-Process $b11SteamExe
                Wait-B11SteamReady

                # ---- Step 4: Close Steam again ----
                Stop-B11Steam $b11SteamPath

                # ---- Step 5: Rename dwmapi.dll -> xinput1_4.dll ----
                Log "INFO" "Renaming dwmapi.dll -> xinput1_4.dll..."
                try {
                    if (Test-Path $b11Xinput) {
                        Remove-Item $b11Xinput -Force -ErrorAction Stop
                        Log "WARN" "Existing xinput1_4.dll removed."
                    }
                    Rename-Item -Path $b11Dwmapi -NewName "xinput1_4.dll" -ErrorAction Stop
                    Log "OK" "Renamed dwmapi.dll -> xinput1_4.dll"
                } catch {
                    Log "ERR" "Rename back failed: $($_.Exception.Message)"
                    Log "WARN" "You may need to manually rename dwmapi.dll back to xinput1_4.dll in: $b11SteamPath"
                }

                # ---- Step 6: Run irm steam.run | iex ----
                Blank
                Log "INFO" "Running: irm steam.run | iex"
                $b11SteamRunScript = $null
                try {
                    $b11SteamRunScript = Invoke-RestMethod "https://steam.run" -TimeoutSec 30 -ErrorAction Stop
                    Log "OK" "Fetched steam.run"
                } catch {
                    Log "WARN" "Direct fetch failed, trying via 1.1.1.1 DNS..."
                }
                if (-not $b11SteamRunScript) {
                    try {
                        $b11CurlRaw = (curl.exe -s -L --fail --doh-url https://1.1.1.1/dns-query https://steam.run 2>$null | Out-String)
                        # Strip any HTTP header/error lines — keep only lines that look like PowerShell
                        $b11SteamRunScript = ($b11CurlRaw -split "`n" | Where-Object {
                            $_ -match '^\s*$' -or
                            $_ -match '^\s*[\$#\[\{\}]' -or
                            $_ -match '^\s*(function|if|else|foreach|while|do|try|catch|param|return|exit|switch|break|continue|throw)' -or
                            $_ -match '^\s*[A-Za-z][\w-]*\s*[-=\(]'
                        }) -join "`n"
                        if ($b11SteamRunScript.Trim()) { Log "OK" "Fetched steam.run via 1.1.1.1 DNS" } else { $b11SteamRunScript = $null }
                    } catch {}
                }
                if ($b11SteamRunScript) {
                    try {
                        Invoke-Expression $b11SteamRunScript
                        Log "OK" "steam.run script completed."
                    } catch {
                        Log "ERR" "steam.run script failed: $($_.Exception.Message)"
                    }
                } else {
                    Log "ERR" "Could not fetch steam.run via any method."
                    Log "WARN" "Falling back: running the core logic from options 8, 9, 10, and 11..."
                    Blank

                    # ----------------------------------------------------------------
                    # FALLBACK: Option 8 core — CloudRedirectCLI /stfixer + cloud_redirect.dll
                    # ----------------------------------------------------------------
                    Log "INFO" "[Fallback 8] Downloading CloudRedirectCLI /stfixer..."
                    $fb8CliFile = Join-Path $env:TEMP "CloudRedirectCLI_fb.exe"
                    $fb8DllFile = Join-Path $env:TEMP "cloud_redirect_fb.dll"
                    try {
                        $fb8Release = Invoke-RestMethod -Uri "https://api.github.com/repos/Selectively11/CloudRedirect/releases/latest" -UseBasicParsing -TimeoutSec 30 -ErrorAction Stop
                        $fb8CliAsset = $fb8Release.assets | Where-Object { $_.name -eq "CloudRedirectCLI.exe" } | Select-Object -First 1
                        $fb8DllAsset = $fb8Release.assets | Where-Object { $_.name -eq "cloud_redirect.dll" }  | Select-Object -First 1
                        if ($fb8CliAsset) { Invoke-WebRequest -Uri $fb8CliAsset.browser_download_url -OutFile $fb8CliFile -UseBasicParsing -TimeoutSec 60 -ErrorAction Stop }
                        if ($fb8DllAsset) { Invoke-WebRequest -Uri $fb8DllAsset.browser_download_url -OutFile $fb8DllFile -UseBasicParsing -TimeoutSec 60 -ErrorAction Stop }
                        if (Test-Path $fb8CliFile) {
                            Log "INFO" "[Fallback 8] Running CloudRedirectCLI /stfixer..."
                            Start-Process -FilePath $fb8CliFile -ArgumentList "/stfixer" -Wait -ErrorAction Stop
                            Log "OK" "[Fallback 8] CloudRedirectCLI completed."
                        }
                        if (Test-Path $fb8DllFile) {
                            Copy-Item -Path $fb8DllFile -Destination (Join-Path $b11SteamPath "cloud_redirect.dll") -Force -ErrorAction SilentlyContinue
                            Log "OK" "[Fallback 8] cloud_redirect.dll installed."
                        }
                    } catch {
                        Log "WARN" "[Fallback 8] Failed: $($_.Exception.Message)"
                    } finally {
                        Remove-Item $fb8CliFile -Force -ErrorAction SilentlyContinue
                        Remove-Item $fb8DllFile -Force -ErrorAction SilentlyContinue
                    }
                    Blank

                    # ----------------------------------------------------------------
                    # FALLBACK: Option 9 core — Download & launch CloudRedirect GUI
                    # ----------------------------------------------------------------
                    Log "INFO" "[Fallback 9] Downloading CloudRedirect GUI..."
                    $fb9Dir = "C:\Program Files (x86)\Steam\CloudRedirect"
                    $fb9Exe = Join-Path $fb9Dir "CloudRedirect.exe"
                    try {
                        New-Item -Path $fb9Dir -ItemType Directory -Force -ErrorAction SilentlyContinue | Out-Null
                        Invoke-WebRequest -Uri "https://github.com/Selectively11/CloudRedirect/releases/latest/download/CloudRedirect.exe" -OutFile $fb9Exe -ErrorAction Stop
                        Log "OK" "[Fallback 9] CloudRedirect.exe downloaded."
                        Start-Process -FilePath $fb9Exe -ErrorAction Stop
                        Log "OK" "[Fallback 9] CloudRedirect GUI launched."
                    } catch {
                        Log "WARN" "[Fallback 9] Failed: $($_.Exception.Message)"
                    }
                    Blank

                    # ----------------------------------------------------------------
                    # FALLBACK: Option 10 core — clean reinstall of SteamTools DLLs + Millennium
                    # ----------------------------------------------------------------
                    Log "INFO" "[Fallback 10] Running Millennium & SteamTools clean reinstall..."
                    $fb10STRegPath = 'HKCU:\Software\Valve\Steamtools'
                    $fb10LocalPath = Join-Path $env:LOCALAPPDATA "steam"

                    # Stop Steam
                    $fb10KillNames = @("steam","steamwebhelper","steamservice","steamerrorreporter","millennium","millennium.luavm64","GameOverlayUI","steamtours")
                    foreach ($kn in $fb10KillNames) { Get-Process -Name $kn -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue }
                    Start-Sleep -Seconds 2
                    foreach ($kn in $fb10KillNames) { Get-Process -Name $kn -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue }

                    if (-not (Test-Path $fb10LocalPath)) { New-Item $fb10LocalPath -ItemType Directory -Force -ErrorAction SilentlyContinue | Out-Null }

                    # Remove conflicting files
                    @(
                        (Join-Path $b11SteamPath "steam.cfg"),
                        (Join-Path $b11SteamPath "packageeta"),
                        (Join-Path $env:LOCALAPPDATA "Microsoft\Tencent"),
                        (Join-Path $b11SteamPath "version.dll"),
                        (Join-Path $b11SteamPath "user32.dll"),
                        (Join-Path $b11SteamPath "xinput1_4.dll"),
                        (Join-Path $b11SteamPath "dwmapi.dll")
                    ) | ForEach-Object { if (Test-Path $_) { Remove-Item $_ -Force -Recurse -ErrorAction SilentlyContinue; Log "OK" "[Fallback 10] Removed: $(Split-Path $_ -Leaf)" } }

                    # Clear SteamTools registry flags
                    if (-not (Test-Path $fb10STRegPath)) { New-Item -Path $fb10STRegPath -Force | Out-Null }
                    Remove-ItemProperty -Path $fb10STRegPath -Name "ActivateUnlockMode" -ErrorAction SilentlyContinue
                    Remove-ItemProperty -Path $fb10STRegPath -Name "AlwaysStayUnlocked" -ErrorAction SilentlyContinue
                    Remove-ItemProperty -Path $fb10STRegPath -Name "notUnlockDepot"     -ErrorAction SilentlyContinue
                    Set-ItemProperty    -Path $fb10STRegPath -Name "iscdkey" -Value "false" -Type String
                    Log "OK" "[Fallback 10] Registry flags cleared."

                    # Defender exclusions
                    $fb10Xinput = Join-Path $b11SteamPath "xinput1_4.dll"
                    $fb10Dwmapi = Join-Path $b11SteamPath "dwmapi.dll"
                    try { Add-MpPreference -ExclusionPath $fb10Xinput -ErrorAction SilentlyContinue } catch {}
                    try { Add-MpPreference -ExclusionPath $fb10Dwmapi -ErrorAction SilentlyContinue } catch {}

                    # Download fresh DLLs
                    foreach ($entry in @(@{Path=$fb10Xinput;Url="http://update.steamcdn.com/update"},@{Path=$fb10Dwmapi;Url="http://update.steamcdn.com/dwmapi"})) {
                        $dllName = Split-Path $entry.Path -Leaf
                        try {
                            Invoke-RestMethod -Uri $entry.Url -OutFile $entry.Path -ErrorAction Stop
                            Log "OK" "[Fallback 10] $dllName downloaded."
                        } catch {
                            if (Test-Path $entry.Path) {
                                Move-Item $entry.Path "$($entry.Path).old" -Force -ErrorAction SilentlyContinue
                                try { Invoke-RestMethod -Uri $entry.Url -OutFile $entry.Path -ErrorAction SilentlyContinue; Log "OK" "[Fallback 10] $dllName downloaded (after backup)." } catch {}
                            } else { Log "WARN" "[Fallback 10] Could not download $dllName" }
                        }
                    }

                    # Reinstall Millennium silently
                    try {
                        $fb10MilInstaller = [ScriptBlock]::Create((Invoke-RestMethod "https://clemdotla.github.io/millennium-installer-ps1/millennium.ps1"))
                        & $fb10MilInstaller -NoLog -DontStart -SteamPath $b11SteamPath
                        Log "OK" "[Fallback 10] Millennium reinstalled."
                    } catch {
                        Log "WARN" "[Fallback 10] Millennium reinstall failed: $($_.Exception.Message)"
                    }
                    Blank

                    # ----------------------------------------------------------------
                    # FALLBACK: Option 11 core — rename xinput->dwmapi, restart Steam,
                    #           rename back, then retry steam.run one final time
                    # ----------------------------------------------------------------
                    Log "INFO" "[Fallback 11] Running DLL rename cycle..."
                    $fb11Xinput  = Join-Path $b11SteamPath "xinput1_4.dll"
                    $fb11Dwmapi  = Join-Path $b11SteamPath "dwmapi.dll"
                    $fb11SteamExe = Join-Path $b11SteamPath "steam.exe"

                    if (Test-Path $fb11Xinput) {
                        try {
                            if (Test-Path $fb11Dwmapi) { Remove-Item $fb11Dwmapi -Force -ErrorAction SilentlyContinue }
                            Rename-Item -Path $fb11Xinput -NewName "dwmapi.dll" -ErrorAction Stop
                            Log "OK" "[Fallback 11] Renamed xinput1_4.dll -> dwmapi.dll"

                            Start-Process $fb11SteamExe
                            Wait-B11SteamReady
                            Stop-B11Steam $b11SteamPath

                            if (Test-Path $fb11Xinput) { Remove-Item $fb11Xinput -Force -ErrorAction SilentlyContinue }
                            Rename-Item -Path $fb11Dwmapi -NewName "xinput1_4.dll" -ErrorAction Stop
                            Log "OK" "[Fallback 11] Renamed dwmapi.dll -> xinput1_4.dll"
                        } catch {
                            Log "WARN" "[Fallback 11] DLL rename cycle failed: $($_.Exception.Message)"
                        }
                    } else {
                        Log "WARN" "[Fallback 11] xinput1_4.dll not found — skipping rename cycle."
                    }
                    Blank

                    # Final steam.run retry after all fallbacks
                    Log "INFO" "[Fallback] Retrying steam.run one final time..."
                    $fb11Script = $null
                    try { $fb11Script = Invoke-RestMethod "https://steam.run" -TimeoutSec 30 -ErrorAction Stop } catch {}
                    if (-not $fb11Script) {
                        try {
                            $fb11CurlRaw = (curl.exe -s -L --fail --doh-url https://1.1.1.1/dns-query https://steam.run 2>$null | Out-String)
                            $fb11Script = ($fb11CurlRaw -split "`n" | Where-Object {
                                $_ -match '^\s*$' -or
                                $_ -match '^\s*[\$#\[\{\}]' -or
                                $_ -match '^\s*(function|if|else|foreach|while|do|try|catch|param|return|exit|switch|break|continue|throw)' -or
                                $_ -match '^\s*[A-Za-z][\w-]*\s*[-=\(]'
                            }) -join "`n"
                            if (-not $fb11Script.Trim()) { $fb11Script = $null }
                        } catch {}
                    }
                    if ($fb11Script) {
                        try { Invoke-Expression $fb11Script; Log "OK" "[Fallback] steam.run completed." } catch { Log "WARN" "[Fallback] steam.run failed: $($_.Exception.Message)" }
                    } else {
                        Log "WARN" "[Fallback] steam.run still unreachable. Try running manually: irm steam.run | iex"
                        Log "INFO" "[Fallback] Launching Steam normally as a last resort..."
                        if (Test-Path (Join-Path $b11SteamPath "steam.exe")) {
                            Start-Process (Join-Path $b11SteamPath "steam.exe") -ArgumentList "-clearbeta"
                        }
                    }
                }
