...
if ($Branch -eq 1 -or $Branch -eq 2) {
    ...
    function Install-Plugin {
        ...
        # Use GetTempPath() instead of $env:TEMP — on systems where the username contains
        # a space, $env:TEMP can contain an 8.3 short path (e.g. "John D~1") which causes
        # file creation to fail. GetTempPath() always returns the full long path.
        $resolvedTemp = [System.IO.Path]::GetTempPath().TrimEnd([char]'\', [char]'/')
        $zipPath = Join-Path $resolvedTemp "$Name.zip"
        ...
        try {
            ...
            # Detect a single top-level folder in the zip (e.g. "ltsteam-plugin-main/")
            # so we can strip it and extract contents directly into $targetDir.
            $topFolders = $zip.Entries |
            ...
            foreach ($entry in $zip.Entries) {
                ...
                $relativeName = $entry.FullName
                if ($stripPrefix -and $relativeName.StartsWith($stripPrefix)) {
                    $relativeName = $relativeName.Substring($stripPrefix.Length)
                }
                if (-not $relativeName) { continue }

                $dest   = Join-Path $targetDir $relativeName
            ...
            # Extract to a temp staging dir, then move contents up to strip the top-level folder
            $stagingDir = Join-Path $resolvedTemp ("lt_staging_" + [System.IO.Path]::GetRandomFileName())
            Expand-Archive -Path $zipPath -DestinationPath $stagingDir -Force
            $stagingChildren = Get-ChildItem $stagingDir
            if ($stagingChildren.Count -eq 1 -and $stagingChildren[0].PSIsContainer) {
                # Single top-level folder: move its contents into $targetDir
                Get-ChildItem $stagingChildren[0].FullName | ForEach-Object {
                    $dst = Join-Path $targetDir $_.Name
                    if (Test-Path $dst) { Remove-Item $dst -Recurse -Force -ErrorAction SilentlyContinue }
                    Move-Item $_.FullName $dst -Force
                }
            } else {
                # No wrapper folder: move everything directly
                $stagingChildren | ForEach-Object {
                    $dst = Join-Path $targetDir $_.Name
                    if (Test-Path $dst) { Remove-Item $dst -Recurse -Force -ErrorAction SilentlyContinue }
                    Move-Item $_.FullName $dst -Force
                }
